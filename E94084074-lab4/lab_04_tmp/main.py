...
class Game:
    ...
    def select(group, x, y):
        ...
        """
        towerlist=group.get()                                 #先將所有的塔輸入到一陣列(towerlist)中
        
        for i in range(len(towerlist)):                       #用迴圈for去判斷滑鼠游標是否在任一塔上
            if(towerlist[i].is_clicked( x, y)==True):         
                towerlist[i].get_selected(True)               #若有，則使該塔的get_selected改為True
            else:
                towerlist[i].get_selected(False)              #若無，則使該塔的get_selected改為False
